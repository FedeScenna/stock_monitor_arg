...
once and then running the two analyses offline against that fresh data:
  ...
  3. Kronos forecast  - 21-day portfolio forecast  -> data/portfolio/kronos_forecast_DATE.csv
  4. Weekly screen    - value + technical screen    -> data/portfolio/weekly_screen_DATE.csv

Steps 3 and 4 run with --no-fetch since steps 1-2 already refreshed everything.
...
def main():
    ...
    if not args.skip_kronos:
        ...
        results.append(_run_script("Step 3/4 - Kronos forecast", "kronos_forecast.py", *kronos_args))
    else:
        print("\nStep 3/4 - Kronos forecast skipped (--skip-kronos)")
    ...
    if not args.skip_screen:
        ...
        results.append(_run_script("Step 4/4 - Weekly screen", "weekly_screen.py", *screen_args))
    else:
        print("\nStep 4/4 - Weekly screen skipped (--skip-screen)")
    ...
    print("\n  Outputs:")
    for pat in (f"kronos_forecast_{today}.csv", f"weekly_screen_{today}.csv"):
        f = PORTFOLIO_DIR / pat
